...
bool skip_7bit_overclocking =
    1; // 7-bit samples have priority over overclocking
int normalscanlines;
int totalscanlines;
...
// If this is true, frame advance will skip over lag frame (i.e. it will emulate
// 2 frames instead of 1)
...
static DECLFR(ANull) { return (X.DB); }
...
uint8 readb(int32 a) { return GetReadHandler(a)(a); }

void writeb(int32 a, uint8 v) { GetWriteHandler(a)(a, v); }
...
static DECLFW(BRAML) { RAM[A] = V; }

static DECLFW(BRAMH) { RAM[A & 0x7FF] = V; }

static DECLFR(ARAML) { return RAM[A]; }
...
void ResetGameLoaded(void) {
  if (GameInfo)
    FCEU_CloseGame();
  EmulationPaused = 0; // mbg 5/8/08 - loading games while paused was bad news.
                       // maybe this fixes it
  GameStateRestore = 0;
  PPU_hook = NULL;
  GameHBIRQHook = NULL;
...
FCEUGI *FCEUI_LoadGameVirtual(const char *name, int OverwriteVidMode,
  ...
  if (!fp) {
    // Although !fp, if the operation was canceled from archive select dialog
    // box, don't show the error message;
  ...
  if (iNESLoad(fullname, fp, OverwriteVidMode)) {
    ...
    if (!lastpal && PAL) {
      ...
      // this won't happen, since we don't autodetect dendy, but maybe someday
      // we will?
...
bool FCEUI_Initialize() {
  ...
  // this is more for precision emulation, instead of entertainment, which is
  // what fceux is all about nowadays
...
/// Skip may be passed in, if FRAMESKIP is #defined, to cause this to emulate
/// more than one frame
void FCEUI_Emulate(uint8 **pXBuf, int32 **SoundBuf, int32 *SoundBufSize,
  ...
  if (frameAdvanceRequested) {
    if (frameAdvance_Delay_count == 0 ||
        frameAdvance_Delay_count >= frameAdvance_Delay)
  ...
  *pXBuf = skip ? 0 : XBuf;
  ...
  // Lots of conditions here.  EmulationPaused & EMULATIONPAUSED_FA must be
  // true.  In addition frameAdvanceLagSkip or lagFlag must be false
  //  When Frame Advance is held, emulator is automatically paused after
  //  emulating one frame (or several lag frames)
  {
    EmulationPaused =
        EMULATIONPAUSED_PAUSED; // restore EMULATIONPAUSED_PAUSED flag and clear
                                // EMULATIONPAUSED_FA flag
  }
}

void FCEUI_CloseGame(void) { FCEU_CloseGame(); }
...
u64 xoroshiro128plus_next() { return 0; }
...
void FCEU_ResetVidSys(void) {
  ...
  normalscanlines = (dendy ? 290 : 240) + newppu; // use flag as number!
  totalscanlines =
      normalscanlines + (overclock_enabled ? postrenderscanlines : 0);
...
int FCEU_TextScanlineOffset(int y) { return FSettings.FirstSLine * 256; }
