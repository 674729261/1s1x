...
uint8 DMCAddressLatch =
    0; // $4012 / Start of DMC waveform is at address $C000 + $40*$xx
uint8 DMCSizeLatch = 0; // $4013 / Length of DMC waveform is $10*$xx + 1 bytes
                        // (128*$xx + 8 samples)
...
static const uint32 NTSCDMCTable[0x10] = {428, 380, 340, 320, 286, 254,
...
static const uint32 PALDMCTable[0x10] = {398, 354, 316, 298, 276, 236, 210, 198,
...
static INLINE void RDoSQ(int x) // Int x decides if this is Square Wave 1 or 2
{
  ...
  // adelikat: Note: the formulat x = x * y /100 does not yield exact results,
  // but is "close enough" and avoids the need for using double vales or
  // implicit cohersion which are slower (we need speed here)
  ampx = x ? FSettings.Square2Volume
           : FSettings.Square1Volume; // TODO OPTIMIZE ME!
  if (ampx != 256)
    amp = (amp * ampx) / 256; // CaH4e3: fixed - setting up maximum volume for
                              // square2 caused complete mute square2 channel
...
static void RDoSQLQ(void) {
  ...
  for (x = 0; x < 2; x++) {
    ...
    // but is "close enough" and avoids the need for using double vales or
    // implicit cohersion which are slower (we need speed here)
    ampx = x ? FSettings.Square1Volume
             : FSettings.Square2Volume; // TODO OPTIMIZE ME!
    if (ampx != 256)
      amp[x] = (amp[x] * ampx) /
               256; // CaH4e3: fixed - setting up maximum volume for square2
                    // caused complete mute square2 channel

    if (!inie[x])
      amp[x] = 0; /* Correct? Buzzing in MM2, others otherwise... */
  ...
  if (!inie[0] && !inie[1]) {
    ...
    for (V = start; V < end; V++) {
      ...
      // tmpamp = wlookup1[ ttable[0][RectDutyCount[0]] +
      // ttable[1][RectDutyCount[1]] ];
...
static void RDoTriangle(void) {
  ...
  if (!lengthcount[2] ||
      !TriCount) { /* Counter is halted, but we still need to output. */
    /*int32 *start = &WaveHi[ChannelBC[2]];
      int32 count = SOUNDTS - ChannelBC[2];
      while(count--)
      {
    //Modify volume based on channel volume modifiers
     *start += (tcout/256*FSettings.TriangleVolume)&(~0xFFFF);  // TODO OPTIMIZE
    ME NOW DAMMIT! start++;
     }*/
    ...
    for (V = ChannelBC[2]; V < SOUNDTS; V++) {
      // Modify volume based on channel volume modifiers
      WaveHi[V] += (tcout / 256 * FSettings.TriangleVolume) &
...
static void RDoTriangleNoisePCMLQ(void) {
  ...
  if (FSettings.TriangleVolume != 256)
    amptab[0] =
        (amptab[0] * FSettings.TriangleVolume) / 256; // TODO OPTIMIZE ME!
...
static void RDoNoise(void) {
  ...
  if (FSettings.NoiseVolume != 256)
    amptab[0] = (amptab[0] * FSettings.NoiseVolume) / 256; // TODO OPTIMIZE ME!
  amptab[0] <<= 16;
  amptab[1] = 0;
...
/* FIXME:  Find out what sound registers get reset on reset.  I know $4001/$4005
   don't, due to that whole MegaMan 2 Game Genie thing.
   */
...
void FCEUSND_Reset(void) {
  ...
  // so, do some ridiculous hackery if a movie's about to play to keep it in
  // sync...
  ...
  if (movieSyncHackOn) {
    if (resetDMCacc) {
      ...
      FCEU_PrintError(
          "Warning: These variables were not found in the save state and will "
          "keep their current value: DMCacc=%d, DMCBitCount=%d\n",
          DMCacc, DMCBitCount);
#endif
    } else {
...
void SetSoundVariables(void) {
  ...
  // nesincsize=(int64)(((int64)1<<17)*(double)(PAL?PAL_CPU:NTSC_CPU)/(FSettings.SndRate
  // * 16));
  nesincsize = (PAL ? 3405019392u : (dendy ? 3632020412u : 3665454545u)) /
               (FSettings.SndRate / 4);
  ...
  // soundtsinc=(uint32)((uint64)(PAL?(long double)PAL_CPU*65536:(long
  // double)NTSC_CPU*65536)/(FSettings.SndRate * 16));
...
void FCEUI_SetSquare1Volume(uint32 volume) { FSettings.Square1Volume = volume; }

void FCEUI_SetSquare2Volume(uint32 volume) { FSettings.Square2Volume = volume; }

void FCEUI_SetNoiseVolume(uint32 volume) { FSettings.NoiseVolume = volume; }

void FCEUI_SetPCMVolume(uint32 volume) { FSettings.PCMVolume = volume; }
