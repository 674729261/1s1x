...
void FCEUD_Update(uint8 *XBuf, int32 *Buffer, int Count) {
  ...
  if (Count) {
    ...
    if (!NoWaiting &&
        (!(eoptions & EO_NOTHROTTLE) || FCEUI_EmulationPaused())) {
      while (SpeedThrottle()) {
        FCEUD_UpdateInput();
      }
    }
 ...
 */
uint64 FCEUD_GetTime() { return io_read(AM_TIMER_UPTIME).us / 1000; }
 ...
 **/
void FCEUD_Message(const char *text) { printf("%s", text); }
 ...
 **/
void FCEUD_PrintError(const char *errormsg) { printf("%s\n", errormsg); }
...
// These are actually fine, but will be unused and overriden by the current UI
// code.
