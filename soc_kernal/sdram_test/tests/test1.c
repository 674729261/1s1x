...
#define TEST_ADDR_START 0xa00ea000
#define TEST_ADDR_END 0xa1dea100

int test_word() {
  uint32_t *start = (uint32_t *)(TEST_ADDR_START);
  uint32_t *end = (uint32_t *)(TEST_ADDR_END);
  for (volatile uint32_t *p = start; p < end; p += 0x100000) {
...
int test_half() {
  ...
  for (volatile uint16_t *p = start; p < end; p += 0x100000) {
...
int test_byte() {
  ...
  for (volatile uint8_t *p = start; p < end; p += 0x100000) {
