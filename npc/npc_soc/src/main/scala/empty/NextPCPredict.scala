...
class NextPCPredict(size_2pow: Int, tag_from: Int, tag_to: Int) extends PrefixedModule {
  ...
  val valid_flags = RegInit(VecInit(Seq.fill(nr_lines)(false.B)))

  val compare_results = VecInit((0 until nr_lines).map { i =>
    content(i).tag === input_tag && valid_flags(i)
  })
  val compare_results_write = VecInit((0 until nr_lines).map { i =>
  ...
  val read_item = Mux1H((0 until nr_lines).map { i =>
  ...
  val write_ptr = RegInit(0.U(size_2pow.W))
    ...
    when(write_hit) {
      for (i <- 0 until nr_lines) {
        when(compare_results_write(i)) {
          ...
          content(i).saturate_count := MuxLookup(content(i).saturate_count, io.init_cnt)(
            Seq(
