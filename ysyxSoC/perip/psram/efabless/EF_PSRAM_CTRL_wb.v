...
module EF_PSRAM_CTRL_wb (
  ...
  localparam ST_IDLE = 2'b0, ST_WAIT = 2'b1, ST_SET_QPI = 2'b10, ST_SPI_WAIT = 2'b11;
  ...
  always @* begin
    case (state)
      ...
      ST_SET_QPI: nstate = ST_SPI_WAIT;

      ST_SPI_WAIT:
      if (ms_done) nstate = ST_IDLE;
      else nstate = ST_SPI_WAIT;
  ...
  assign qpi_set = (state == ST_SET_QPI);
  ...
  PSRAM_QPI_SETTER u_PSRAM_QPI_SETTER (
      .clk   (clk_i),
      .rst_n (~rst_i),
      .fire  (qpi_set),
      .done  (ms_done),
  ...
  wire is_initing;
  assign is_initing = state == ST_SET_QPI || state == ST_SPI_WAIT;

  assign sck = is_initing ? ms_sck : (wb_we ? mw_sck : mr_sck);
  assign ce_n = is_initing ? ms_ce_n : (wb_we ? mw_ce_n : mr_ce_n);
  assign dout = is_initing ? ms_dout : (wb_we ? mw_dout : mr_dout);
  assign douten = is_initing ? {4{ms_doe}} : (wb_we ? {4{mw_doe}} : {4{mr_doe}});
