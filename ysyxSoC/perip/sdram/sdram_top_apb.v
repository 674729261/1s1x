module sdram_top_apb (
  ...
  localparam ST_WAIT_ACCEPT = 2'b01;
  localparam ST_WAIT_ACK = 2'b10;
  reg  [1:0] state;
  wire       req_accept;
  ...
  always @(posedge clock) begin
    if (reset) state <= ST_IDLE;
      ...
      case (state)
        ST_IDLE:
        state <= (is_read || is_write ? (req_accept ? ST_WAIT_ACK : ST_WAIT_ACCEPT) : ST_IDLE);
  ...
  sdram_axi_core #(
      ...
      .sdram_we_o               (sdram_we),
      .sdram_dqm0_low_o         (sdram_dqm0_low),
      .sdram_dqm0_high_o        (sdram_dqm0_high),
      .sdram_dqm1_low_o         (sdram_dqm1_low),
      .sdram_dqm1_high_o        (sdram_dqm1_high),
