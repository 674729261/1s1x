...
class ysyxSoCASIC(implicit p: Parameters) extends LazyModule {
  ...
  val lsdram_apb =
    if (!Config.sdramUseAXI) Some(LazyModule(new APBSDRAM(sdramAddressSet)))
    else None
  val lsdram_axi =
    if (Config.sdramUseAXI) Some(LazyModule(new AXI4SDRAM(sdramAddressSet)))
    else None
  ...
  class Impl extends LazyModuleImp(this) with DontTouch {
    ...
    val sdramBundle =
      if (Config.sdramUseAXI) lsdram_axi.get.module.sdram_bundle
...
class ysyxSoCFull(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) with DontTouch {
    ...
    val sdram0_low = Module(new sdramChisel)
    sdram0_low.io.a := masic.sdram.a
    sdram0_low.io.ba := masic.sdram.ba
    sdram0_low.io.cas := masic.sdram.cas
    sdram0_low.io.cke := masic.sdram.cke
    sdram0_low.io.clk := masic.sdram.clk
    sdram0_low.io.cs := masic.sdram.cs
    sdram0_low.io.ras := masic.sdram.ras
    sdram0_low.io.we := masic.sdram.we
    sdram0_low.io.dq <> masic.sdram.dq0_low
    sdram0_low.io.dqm := masic.sdram.dqm0_low

    val sdram0_high = Module(new sdramChisel)
    sdram0_high.io.a := masic.sdram.a
    sdram0_high.io.ba := masic.sdram.ba
    sdram0_high.io.cas := masic.sdram.cas
    sdram0_high.io.cke := masic.sdram.cke
    sdram0_high.io.clk := masic.sdram.clk
    sdram0_high.io.cs := masic.sdram.cs
    sdram0_high.io.ras := masic.sdram.ras
    sdram0_high.io.we := masic.sdram.we
    sdram0_high.io.dq <> masic.sdram.dq0_high
    sdram0_high.io.dqm := masic.sdram.dqm0_high

    val sdram1_low = Module(new sdramChisel)
    sdram1_low.io.a := masic.sdram.a
    sdram1_low.io.ba := masic.sdram.ba
    sdram1_low.io.cas := masic.sdram.cas
    sdram1_low.io.cke := masic.sdram.cke
    sdram1_low.io.clk := masic.sdram.clk
    sdram1_low.io.cs := masic.sdram.cs
    sdram1_low.io.ras := masic.sdram.ras
    sdram1_low.io.we := masic.sdram.we
    sdram1_low.io.dq <> masic.sdram.dq1_low
    sdram1_low.io.dqm := masic.sdram.dqm1_low

    val sdram1_high = Module(new sdramChisel)
    sdram1_high.io.a := masic.sdram.a
    sdram1_high.io.ba := masic.sdram.ba
    sdram1_high.io.cas := masic.sdram.cas
    sdram1_high.io.cke := masic.sdram.cke
    sdram1_high.io.clk := masic.sdram.clk
    sdram1_high.io.cs := masic.sdram.cs
    sdram1_high.io.ras := masic.sdram.ras
    sdram1_high.io.we := masic.sdram.we
    sdram1_high.io.dq <> masic.sdram.dq1_high
    sdram1_high.io.dqm := masic.sdram.dqm1_high
