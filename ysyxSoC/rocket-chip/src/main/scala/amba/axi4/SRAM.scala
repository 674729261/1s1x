...
/** AXI4 slave device to provide a RAM storage
  *
  * Setting wcorrupt=true is not enough to enable the w.user field You must also
  * list AMBACorrupt in your master's requestFields
  *
  * @param address
  *   address range
  * @param cacheable
  *   whether this ram is cacheable
  * @param executable
  *   whether this ram is executable
  * @param beatBytes
  *   number of bytes in each beat
  * @param devName
  *   optional device name
  * @param errors
  *   address ranges where all access should fail
  * @param wcorrupt
  *   enable AMBACorrupt in w.user
  */
...
class AXI4RAM(
    ...
    extends DiplomaticSRAM(address, beatBytes, devName) {
      ...
      AXI4SlavePortParameters(
        Seq(
          AXI4SlaveParameters(
            ...
            resources = resources,
            regionType =
              if (cacheable) RegionType.UNCACHED else RegionType.IDEMPOTENT,
  ...
  class Impl extends LazyModuleImp(this) with HasJustOneSeqMem {
    ...
    val corrupt =
      if (edgeIn.bundle.requestFields.contains(AMBACorrupt))
        Some(SyncReadMem(1 << mask.filter(b => b).size, UInt(2.W)))
      else None

    val r_addr = Cat(
      (mask zip (in.ar.bits.addr >> log2Ceil(beatBytes)).asBools)
        .filter(_._1)
        .map(_._2)
        .reverse
    )
    val w_addr = Cat(
      (mask zip (in.aw.bits.addr >> log2Ceil(beatBytes)).asBools)
        .filter(_._1)
        .map(_._2)
        .reverse
    )
    ...
    val wdata = VecInit.tabulate(beatBytes) { i =>
      in.w.bits.data(8 * (i + 1) - 1, 8 * i)
    }
    ...
    in.r.bits.resp := Mux(
      r_sel1,
      Mux(rcorrupt, AXI4Parameters.RESP_SLVERR, AXI4Parameters.RESP_OKAY),
      AXI4Parameters.RESP_DECERR
    )
