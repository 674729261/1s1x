 ...
 * 2022-01-07     Gabriel      move some __on_rt_xxxxx_hook to dedicated c
 * source files 2022-01-12     Meco Man     remove RT_THREAD_BLOCK 2022-04-20
 * Meco Man     change version number to v4.1.1 2022-04-21     THEWON       add
 * macro RT_VERSION_CHECK 2022-06-29     Meco Man     add RT_USING_LIBC and
 * standard libc headers 2022-08-16     Meco Man     change version number to
 * v5.0.0 2022-09-12     Meco Man     define rt_ssize_t 2022-12-20     Meco Man
 * add const name for rt_object 2023-04-01     Chushicheng  change version
 * number to v5.0.1
 */
...
#define RTTHREAD_VERSION                                                       \
  RT_VERSION_CHECK(RT_VERSION_MAJOR, RT_VERSION_MINOR, RT_VERSION_PATCH)
...
#define INIT_EXPORT(fn, level)                                                 \
  ...
  __declspec(allocate(                                                         \
      "rti_fn$f")) rt_used const struct rt_init_desc __rt_init_msc_##fn = {    \
...
#define INIT_EXPORT(fn, level)                                                 \
  const char __rti_##fn##_name[] = #fn;                                        \
  rt_used const struct rt_init_desc __rt_init_desc_##fn rt_section(            \
...
#define RT_THREAD_SUSPEND_INTERRUPTIBLE                                        \
  (RT_THREAD_SUSPEND_MASK) /**< Suspend interruptable 0x4 */
#define RT_THREAD_SUSPEND RT_THREAD_SUSPEND_INTERRUPTIBLE
#define RT_THREAD_SUSPEND_KILLABLE                                             \
  (RT_THREAD_SUSPEND_MASK |                                                    \
   RT_SIGNAL_COMMON_WAKEUP_MASK) /**< Suspend with killable 0x6 */
#define RT_THREAD_SUSPEND_UNINTERRUPTIBLE                                      \
  (RT_THREAD_SUSPEND_MASK | RT_SIGNAL_COMMON_WAKEUP_MASK |                     \
   RT_SIGNAL_KILL_WAKEUP_MASK) /**< Suspend with uninterruptable 0x7 */
...
struct rt_thread {
  ...
  rt_ubase_t from_context_p;
  rt_ubase_t to_context_p;
};
...
struct rt_messagequeue {
  ...
  rt_list_t suspend_sender_thread; /**< sender thread suspended on this message
                                      queue */
};
...
#define RT_DEVICE_CTRL_BLK_PARTITION                                           \
  (RT_DEVICE_CTRL_BASE(Block) + 5) /**< get block device partition */
...
struct rt_device_ops {
  ...
  rt_ssize_t (*read)(rt_device_t dev, rt_off_t pos, void *buffer,
                     rt_size_t size);
  rt_ssize_t (*write)(rt_device_t dev, rt_off_t pos, const void *buffer,
                      rt_size_t size);
