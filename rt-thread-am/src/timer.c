 ...
 */
static void _timer_init(rt_timer_t timer, void (*timeout)(void *parameter),
 ...
 */
static rt_err_t _timer_list_next_timeout(rt_list_t timer_list[],
...
void rt_timer_dump(rt_list_t timer_heads[]) {
  ...
  for (list = timer_heads[RT_TIMER_SKIP_LIST_LEVEL - 1].next;
       list != &timer_heads[RT_TIMER_SKIP_LIST_LEVEL - 1]; list = list->next) {
    struct rt_timer *timer =
        rt_list_entry(list, struct rt_timer, row[RT_TIMER_SKIP_LIST_LEVEL - 1]);
 ...
 *
 *             NOTE: The max timeout tick should be no more than (RT_TICK_MAX/2
 * - 1).
 *
 ...
 *
 * @param flag is the flag of timer. Timer will invoke the timeout function
 * according to the selected values of flag, if one or more of the following
 * flags is set.
 *
 ...
 *
 *        NOTE:
 *        You can use multiple values with "|" logical operator.  By default,
 * system will use the RT_TIME_FLAG_HARD_TIMER.
 *
...
rt_err_t rt_timer_start(rt_timer_t timer) {
  ...
  if (timer->parent.flag & RT_TIMER_FLAG_SOFT_TIMER) {
    ...
    if ((_soft_timer_status == RT_SOFT_TIMER_IDLE) &&
        ((_timer_thread.stat & RT_THREAD_SUSPEND_MASK) ==
...
#ifdef RT_USING_TIMER_SOFT
/**
 * @brief This function will check software-timer list, if a timeout event
 * happens, the corresponding timeout function will be invoked.
 */
...
void rt_system_timer_thread_init(void) {
  ...
  rt_thread_init(&_timer_thread, "timer", _timer_thread_entry, RT_NULL,
                 &_timer_thread_stack[0], sizeof(_timer_thread_stack),
