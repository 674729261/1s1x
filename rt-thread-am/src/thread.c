 ...
 * 2021-02-24     Meco Man     rearrange rt_thread_control() - schedule the
 * thread when close it 2021-11-15     THEWON       Remove duplicate work
 * between idle and _thread_exit 2021-12-27     Meco Man     remove
 * .init_priority 2022-01-07     Gabriel      Moving __on_rt_xxxxx_hook to
 * thread.c 2022-01-24     THEWON       let rt_thread_sleep return thread->error
 * when using signal 2022-10-15     Bernard      add nested mutex feature
 */
...
/**
 * @brief   This function is the timeout function for thread, normally which is
 * invoked when thread is timeout to wait some resource.
 *
...
static rt_err_t _thread_init(struct rt_thread *thread, const char *name,
  ...
  rt_timer_init(&(thread->thread_timer), thread->parent.name, _thread_timeout,
 ...
 * function is successfully executed. If the return value is any other values,
 * it means this operation failed.
 */
rt_err_t rt_thread_init(struct rt_thread *thread, const char *name,
  ...
  return _thread_init(thread, name, entry, parameter, stack_start, stack_size,
                      priority, tick);
}
...
/**
 * @brief   This function will detach a thread. The thread object will be
 * removed from thread queue and detached/deleted from the system object
 * management.
 *
...
#ifdef RT_USING_HEAP
/**
 * @brief   This function will create a thread object and allocate thread object
 * memory. and stack.
 *
 * @param   name is the name of thread, which shall be unique.
 *
 ...
 * @param   tick is the time slice if there are same priority thread.
 *
 * @return  If the return value is a rt_thread structure pointer, the function
 * is successfully executed. If the return value is RT_NULL, it means this
 * operation failed.
 */
rt_thread_t rt_thread_create(const char *name, void (*entry)(void *parameter),
  ...
  _thread_init(thread, name, entry, parameter, stack_start, stack_size,
               priority, tick);
...
/**
 * @brief   This function will delete a thread. The thread object will be
 * removed from thread queue and deleted from system object management in the
 * idle thread.
 *
 * @param   thread is the thread to be deleted.
 *
...
/**
 * @brief   This function will let current thread yield processor, and scheduler
 * will choose the highest thread to run. After yield processor, the current
 * thread is still in READY state.
 *
 * @return  Return the operation status. If the return value is RT_EOK, the
 * function is successfully executed. If the return value is any other values,
 * it means this operation failed.
...
/**
 * @brief   This function will let current thread sleep for some ticks. Change
 * current thread state to suspend, when the thread timer reaches the tick
 * value, scheduler will awaken this thread.
 *
 ...
 * it means this operation failed.
 */
rt_err_t rt_thread_delay(rt_tick_t tick) { return rt_thread_sleep(tick); }
RTM_EXPORT(rt_thread_delay);

/**
 * @brief   This function will let current thread delay until (*tick +
 * inc_tick).
 *
...
/**
 * @brief   This function will control thread behaviors according to control
 * command.
 *
 ...
 *
 *              RT_THREAD_CTRL_CHANGE_PRIORITY for changing priority level of
 * thread.
 *
...
static void rt_thread_set_suspend_state(struct rt_thread *thread,
                                        int suspend_flag) {
...
/**
 * @brief   This function will suspend the specified thread and change it to
 * suspend state.
 *
 * @note    This function ONLY can suspend current thread itself.
 *              rt_thread_suspend(rt_thread_self());
 *
 *          Do not use the rt_thread_suspend to suspend other threads. You have
 * no way of knowing what code a thread is executing when you suspend it. If you
 * suspend a thread while sharing a resouce with other threads and occupying
 * this resouce, starvation can occur very easily.
 *
...
rt_err_t rt_thread_suspend_with_flag(rt_thread_t thread, int suspend_flag) {
  ...
  if ((stat != RT_THREAD_READY) && (stat != RT_THREAD_RUNNING)) {
    RT_DEBUG_LOG(RT_DEBUG_THREAD,
                 ("thread suspend: thread disorder, 0x%2x\n", thread->stat));
    return -RT_ERROR;
  }
...
void rt_thread_wakeup_set(struct rt_thread *thread, rt_wakeup_func_t func,
                          void *user_data) {
  register rt_base_t temp;
 ...
 */
rt_err_t rt_thread_get_name(rt_thread_t thread, char *name,
                            rt_uint8_t name_size) {
  return (thread == RT_NULL)
             ? -RT_EINVAL
             : rt_object_get_name(&thread->parent, name, name_size);
}
RTM_EXPORT(rt_thread_get_name);

void rt_thread_exit(void) { _thread_exit(); }
